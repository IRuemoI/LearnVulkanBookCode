...
void VulkanDemoApp::createRenderPassSelf()
{
    ...
    rp_info.dependencyCount = 0;
    rp_info.pDependencies = nullptr;
    VkResult result = vkCreateRenderPass(device, &rp_info, nullptr, &renderPassSelf); //创建渲染过程
    assert(result == VK_SUCCESS); //检查是否创建成功
    for (int i = 0; i < OUT_TEX_COUNT; i++)
    {
        //遍历所有颜色附件
    ...
    rp_begin_self.renderPass = renderPassSelf; //指定要启动的渲染过程
...
void VulkanDemoApp::createFrameBufferSelf() //创建帧缓冲的方法（服务于 MRT）
{
    ...
    fb_info.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
    fb_info.pNext = nullptr;
    fb_info.renderPass = renderPassSelf; //指定渲染过程
...
{
    rp_begin_self.framebuffer = selfTexFramebuffer; //将自定义帧缓冲设置为渲染过程的当前帧缓冲
