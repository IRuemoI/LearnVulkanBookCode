...
void VulkanDemoApp::drawSceneToTex()
{
    ...
    rp_begin.framebuffer = selfTexFramebuffer; //将镜像绘制专用帧缓冲设置为渲染过程的当前帧缓冲
...
void VulkanDemoApp::drawSceneToScreen()
{
    ...
    VkResult result = vkAcquireNextImageKHR(device, swapChain, UINT64_MAX, imageAcquiredSemaphore, VK_NULL_HANDLE,
                                            &currentBuffer);
    rp_begin.framebuffer = framebuffers[currentBuffer]; //指定渲染过程使用的帧缓冲
    ...
    borderObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[
                                   TextureManager::getVkDescriptorSetIndex("../textures/1310_mirror.bntex")])); //绘制镜子边框
...
void VulkanDemoApp::drawThings()
{
    ...
    homeObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[
                                 TextureManager::getVkDescriptorSetIndex("../textures/1310_skybox.bntex")]));
    ...
    dengObjForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                             &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1310_deng.bntex")]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
