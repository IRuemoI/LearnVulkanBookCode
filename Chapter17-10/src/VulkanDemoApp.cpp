...
void VulkanDemoApp::createRenderPass()
{
    ...
    for (int i = 0; i < COLOR_ATTACH_COUNT; i++)
    {
        ...
        attachments[i].storeOp = (i == 3) ? VK_ATTACHMENT_STORE_OP_STORE : VK_ATTACHMENT_STORE_OP_DONT_CARE;
        //设置渲染过程结束时附件的存储操作
        attachments[i].finalLayout = (i == 3)
                                         ? VK_IMAGE_LAYOUT_PRESENT_SRC_KHR
                                         : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; //设置结束时的最终布局
...
void VulkanDemoApp::drawObject()
{
    ...
    vkCmdBindDescriptorSets(cmdBuffer, //绑定描述集
                            VK_PIPELINE_BIND_POINT_GRAPHICS, sqsL->pipelineLayout, 0, 1, &(sqsL->descSet[0]), 0,
                            nullptr);
