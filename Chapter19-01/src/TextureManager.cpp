...
std::vector<std::string> TextureManager::texNames = {
    back_texture, dianjistart_texture, resume_texture, guanyu_texture, about_texture, steps_texture, exitbtn_texture,
    grass_texture, cft_texture, floor_texture, box_texture, zft_texture, dici_texture, jiazi_texture, cftshadow_texture,
    qizi_texture, qizigun_texture, tanshe_texture, musicbtn_texture, musicclosebtn_texture
};
std::vector<std::string> TextureManager::texNamesForCommonTexLight = {
    grass_texture, cft_texture, floor_texture, box_texture, zft_texture, dici_texture, jiazi_texture, cftshadow_texture,
    qizi_texture, qizigun_texture, tanshe_texture
};
std::vector<std::string> TextureManager::texNamesForDashboard2D = {
    back_texture, resume_texture, guanyu_texture, dianjistart_texture, about_texture, steps_texture, exitbtn_texture,
    musicbtn_texture, musicclosebtn_texture
};
...
void TextureManager::initSampler(VkDevice& device, VkPhysicalDevice& gpu)
{
    ...
    for (int i = 0; i < SAMPLER_COUNT; i++)
    {
        ...
        VkResult result = vkCreateSampler(device, &samplerCreateInfo, NULL, &samplerTexture);
        samplerList.push_back(samplerTexture);
    }
}

void TextureManager::init_DXT5_BC3_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu,
                                            VkPhysicalDeviceMemoryProperties& memoryroperties,
                                            VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format,
                                            TexDataObject* ctdo)

{
    ...
    printf("TextureManager %s %s\n", (needStaging ? "cannot use linear tiling" : "can use linear tiling"));
    if (needStaging)
    {
        VkBuffer tempBuf;
        ...
        buf_info.flags = 0;
        VkResult result = vkCreateBuffer(device, &buf_info, NULL, &tempBuf);
        ...
        {
            printf("Memory type determination failed!\\n");
        }
        VkDeviceMemory memTemp;
        result = vkAllocateMemory(device, &alloc_info, NULL, &memTemp);
        assert(result == VK_SUCCESS);
        uint8_t* pData;
        ...
        result = vkCreateImage(device, &image_create_info, NULL, &textureImage);
        assert(result == VK_SUCCESS);
        ...
        flag = memoryTypeFromProperties(memoryroperties, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                        &mem_alloc.memoryTypeIndex);
        VkDeviceMemory textureMemory;
        result = vkAllocateMemory(device, &mem_alloc, NULL, &(textureMemory));
        ...
        setImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        vkCmdCopyBufferToImage(cmdBuffer, tempBuf, textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                               &bufferCopyRegion);
        setImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
        ...
        vkDestroyBuffer(device, tempBuf, NULL);
        vkFreeMemory(device, memTemp, NULL);
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu,
                                  ...
                                  VkQueue& queueGraphics)
{
    ...
    {
        TexDataObject* ctdo = FileUtil::load_DXT5_BC3_TexData(texNames[i]); //¼ÓÔØÎÆÀíÎÄ¼þÊý¾Ý
        printf("\\n%s w %d h %d\\n", texNames[i].c_str(), ctdo->width, ctdo->height); // Print texture data info
        init_DXT5_BC3_Textures(texNames[i], device, gpu, memoryroperties, cmdBuffer, queueGraphics,
                               VK_FORMAT_BC3_UNORM_BLOCK, ctdo); //¼ÓÔØÑ¹ËõÎÆÀí 
    }
}
...
void TextureManager::deatroyTextures(VkDevice& device)
{
    for (int i = 0; i < SAMPLER_COUNT; i++)
    {
        vkDestroySampler(device, samplerList[i], NULL);
    }
    for (int i = 0; i < texNames.size(); i++)
    {
        vkDestroyImageView(device, viewTextureList[texNames[i]], NULL);
        vkDestroyImage(device, textureImageList[texNames[i]], NULL);
        vkFreeMemory(device, textureMemoryList[texNames[i]], NULL);
    }
}
