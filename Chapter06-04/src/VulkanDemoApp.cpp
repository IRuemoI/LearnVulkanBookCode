...
void VulkanDemoApp::drawObject()
{
    ...
    if (smallType == 0)
    {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[
                             TextureManager::getVkDescriptorSetIndex("../textures/0604_256Nearest.bntex")])); //绘制小纹理矩形
    }
    ...
    else
    {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(
                             "../textures/0604_256Linear.bntex")])); //绘制小纹理矩形
    }
    ...
    if (bigType == 0)
    {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[
                              TextureManager::getVkDescriptorSetIndex("../textures/0604_32Nearest.bntex")])); //绘制大纹理矩形
    }
    ...
    else
    {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(
                              "../textures/0604_32Linear.bntex")])); //绘制大纹理矩形
    }
    MatrixState3D::popMatrix(); //恢复现场
