...
TextureRect::TextureRect(float* vdataIn, int vdataByteCount, int vCountIn, int* indexDataIn, int indexDataByteCount,
                         int indexCountIn, VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryProperties)
{
...
void TextureRect::createVertexDataBuffer(int vdataByteCount, VkDevice& device,
                                         VkPhysicalDeviceMemoryProperties& memoryProperties)
{
    ...
    bool flag = memoryTypeFromProperties(memoryProperties, vdataMem_reqs.memoryTypeBits, requirements_mask,
...
void TextureRect::createVertexIndexDataBuffer(int indexDataByteCount, VkDevice& device,
                                              VkPhysicalDeviceMemoryProperties& memoryProperties)
{
    VkBufferCreateInfo buf_info = {};
    buf_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
...
void TextureRect::calVerticesNormalAndTangent()
{
    //计算顶点坐标与法向量的方法
...
float TextureRect::findHeight(float x, float z)
{
    //计算3 个波影响后顶点高度值
...
void TextureRect::drawSelf(VkCommandBuffer& cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
