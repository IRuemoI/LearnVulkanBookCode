...
void VulkanDemoApp::createDrawableObject()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        LYobjParticle[i] = new ParticleForDraw(particleVector[i]->vdata, particleVector[i]->dataByteCount,
                                               particleVector[i]->vCount, device, memoryProperties);
    }
}
...
void VulkanDemoApp::flushUniformBuffer() // 将当前帧相关数据送入一致变量缓冲
{
    ...
    uint8_t* pData;
    VkResult result = vkMapMemory(device, sqsParticle->memUniformBuf, 0, sqsParticle->bufferByteCount, 0,
                                  (void**)&pData);
    assert(result == VK_SUCCESS);
    ...
    result = vkMapMemory(device, sqsParticleThree->memUniformBuf, 0, sqsParticleThree->bufferByteCount, 0,
                         (void**)&pData);
    assert(result == VK_SUCCESS);
    memcpy(pData, uniformData, sqsParticleThree->bufferByteCount);
    vkUnmapMemory(device, sqsParticleThree->memUniformBuf);

    result = vkMapMemory(device, sqsParticleFour->memUniformBuf, 0, sqsParticleFour->bufferByteCount, 0,
                         (void**)&pData);
    assert(result == VK_SUCCESS);
...
void VulkanDemoApp::drawObject()
{
    ...
    vkCmdBeginRenderPass(cmdBuffer, &rp_begin, VK_SUBPASS_CONTENTS_INLINE); // 启动渲染过程

    //绘制地板
    MatrixState3D::pushMatrix();
    MatrixState3D::rotate(90, 1, 0, 0);
    LYobjDiban->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1214_wall.bntex")]));
    MatrixState3D::popMatrix();
    //绘制架子
    const float penDistance = 7.0f;
    MatrixState3D::pushMatrix();
    MatrixState3D::translate(penDistance, 0, penDistance);
    LYobjB->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                     &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1214_brazier.bntex")]));
    MatrixState3D::popMatrix();
    ...
    for (auto oneP : particleVector)
    {
        ...
        {
        case 0:
            LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticle->pipelineLayout, sqsParticle->pipeline,
                                              &(sqsParticle->descSet[TextureManager::getVkDescriptorSetIndex(
                                                  ("../textures/1214_fire.bntex"))]), oneP->id);
            break;
        case 1:
            LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleTwo->pipelineLayout, sqsParticleTwo->pipeline,
                                              &(sqsParticleTwo->descSet[TextureManager::getVkDescriptorSetIndex(
                                                  ("../textures/1214_fire.bntex"))]), oneP->id);
            break;
        case 2:
            LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleThree->pipelineLayout, sqsParticleThree->pipeline,
                                              &(sqsParticleThree->descSet[TextureManager::getVkDescriptorSetIndex(
                                                  ("../textures/1214_fire.bntex"))]), oneP->id);
            break;
        case 3:
            LYobjParticle[oneP->id]->drawSelf(cmdBuffer, sqsParticleFour->pipelineLayout, sqsParticleFour->pipeline,
                                              &(sqsParticleFour->descSet[TextureManager::getVkDescriptorSetIndex(
                                                  ("../textures/1214_fire.bntex"))]), oneP->id);
            break;
        }
        MatrixState3D::popMatrix();
