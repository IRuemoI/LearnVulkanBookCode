...
void VulkanDemoApp::drawObject()
{
    ...
    if (isSwap == 0)
    {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    }
    else if (isSwap == 1)
    {
        texTri->drawSelf(cmdBuffer, sqsXY->pipelineLayout, sqsXY->pipeline,
                         &(sqsXY->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    }
    else if (isSwap == 2)
    {
        texTri->drawSelf(cmdBuffer, sqsXie->pipelineLayout, sqsXie->pipeline,
                         &(sqsXie->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1101_vulkan.bntex")]));
    }
