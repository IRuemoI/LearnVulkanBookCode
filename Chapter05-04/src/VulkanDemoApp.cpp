...
void VulkanDemoApp::createDrawableObject()
{
    ...
    ballForDraw = new DrawableObjectCommonLight(BallData::vdata, BallData::dataByteCount, BallData::vCount, device,
                                                memoryProperties); //创建绘制用球
}
...
void VulkanDemoApp::flushUniformBuffer() // 将当前帧相关数据送入一致变量缓冲
{
    float vertexUniformData[12] = {
        ...
        LightManager::lightSpecularA //镜面光强度RGBA 分量值
    };
