...
std::span<const uint32_t> ShaderQueueSuit_Common::compileGLSLtoSPIRV(const char* shaderFilePath,
                                                                     shaderc_shader_kind shader_kind) const
{
    ...
    std::cout << errorInfo;
    std::span<const uint32_t> shader_spv = {
        shaderResult.begin(), size_t(shaderResult.end() - shaderResult.begin()) * 4
    };
    return shader_spv;
}
