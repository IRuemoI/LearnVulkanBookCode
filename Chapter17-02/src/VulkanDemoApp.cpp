...
void VulkanDemoApp::flushUniformBuffer()
{
    ...
    float qiuUniformData[27] = {
        ...
        LightManager::lightSpecularR, LightManager::lightSpecularG, LightManager::lightSpecularB,
        LightManager::lightSpecularA,
        PlanetConstant::perAngle, PlanetConstant::rotateAngle, PlanetConstant::groupNumber,
        PlanetConstant::oneGroupNumber, PlanetConstant::distanceDai, PlanetConstant::linWidthSpan,
...
void VulkanDemoApp::flushTexToDesSet()
{
    ...
    for (int i = 0; i < TextureManager::doubleStone.size() / 2; i++)
    {
        ...
        sqsQiu->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i * 2 + 0]]);
        //加载第一个纹理
        sqsQiu->writes[2].dstSet = sqsQiu->descSet[i];
        sqsQiu->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i * 2 + 1]]);
        //加载第二个纹理
        ...
        sqsQiuTwo->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i * 2 + 0]]);
        //加载第一个纹理
        sqsQiuTwo->writes[2].dstSet = sqsQiuTwo->descSet[i];
        sqsQiuTwo->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleStone[i * 2 + 1]]);
        //加载第二个纹理
...
void VulkanDemoApp::drawObject()
{
    ...
    objObject->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                        &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1702_qiu.bntex")]));
