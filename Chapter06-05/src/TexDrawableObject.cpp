...
TexDrawableObject::TexDrawableObject(float* vdataIn, int dataByteCount, int vCountIn, VkDevice& device,
...
void TexDrawableObject::drawSelf(VkCommandBuffer& cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
                                 VkDescriptorSet* desSetPointer, float lodLevel)
{
    ...
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(float) * 16, pushConstantDataVertex);
    //将最终变换矩阵数据送入推送常量
    pushConstantDataFrag[0] = lodLevel; //纹理采样细节级别数据
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(float) * 16, sizeof(float) * 1,
                       pushConstantDataFrag); //将纹理采样细节级别数据送入推送常量
