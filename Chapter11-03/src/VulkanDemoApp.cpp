...
void VulkanDemoApp::createDrawableObject()
{
    ...
    texTri = new TexDrawableObject(ObjectData::vdata, ObjectData::dataByteCount, ObjectData::vCount, device,
                                   memoryProperties);

    FlatData::genVertexData(); //初始化水面数据
    waterForDraw = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device,
                                         memoryProperties); //水面绘制者
    TreeTrunkData::genVertexData(Constant::bottom_radius, Constant::joint_height, Constant::joint_num,
                                 Constant::joint_available_num); //初始化树干数据
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        treeControl->trunkVector.push_back(
            new TexTrunkDrawableObject(TreeTrunkData::vdata, TreeTrunkData::dataByteCount, TreeTrunkData::vCount,
                                       device, memoryProperties, treePostion[i][0], treePostion[i][1],
                                       treePostion[i][2])); //创建树干
    }
    for (int i = 0; i < treeControl->trunkVector.size(); i++)
    {
        //循环创建树叶
        for (int j = 0; j < 6; j++)
        {
            treeControl->leafVector.push_back(
                new TexLeafDrawableObject(TreeLeaves::vdata, TreeLeaves::dataByteCount, TreeLeaves::vCount, device,
                                          memoryProperties, treePostion[i][0], treePostion[i][1], treePostion[i][2],
                                          j * 60, j));
        }
    }
    ...
    skyForDraw = new TexDrawableObject(SkyBallData::vdata, SkyBallData::dataByteCount, SkyBallData::vCount, device,
                                       memoryProperties); //天空盒
}
...
void VulkanDemoApp::flushTexToDesSet() const // 更新绘制用描述集的方法
{
    ...
    for (int i = 0; i < TextureManager::doubleNames.size() / 2; i++)
    {
        ...
        sqsDou->writes[1].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleNames[i * 2 + 0]]);
        //加载第一个纹理
        sqsDou->writes[2].dstSet = sqsDou->descSet[i];
        sqsDou->writes[2].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::doubleNames[i * 2 + 1]]);
        //加载第二个纹理
...
void VulkanDemoApp::drawObject()
{
    ...
    waterForDraw->drawSelf(cmdBuffer, sqsXie->pipelineLayout, sqsXie->pipeline,
                           &(sqsXie->descSet[TextureManager::getVkDescriptorWaterSetIndex(
                               "../textures/1103_water.bntex")]));
    MatrixState3D::popMatrix();

    MatrixState3D::pushMatrix(); //绘制天空穹
    skyForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1103_sky.bntex")]));
    MatrixState3D::popMatrix();
