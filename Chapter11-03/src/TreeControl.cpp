...
void TreeControl::drawSelf(float bend_RIn, float direction_degreeIn)
{
    //绘制树的方法
    for (int i = 0; i < trunkVector.size(); i++)
    {
        //遍历树干部件列表
    ...
    {
        //绘制树叶
        float* position = calculateLastPosition(bend_RIn, direction_degreeIn, leafVector[i]->postionX,
                                                Constant::TREE_HEIGHT, leafVector[i]->postionZ); //计算树叶最终姿态
        ...
        leafVector[i]->drawSelf(VulkanDemoApp::cmdBuffer, VulkanDemoApp::sqsBlend->pipelineLayout,
                                VulkanDemoApp::sqsBlend->pipeline, &(VulkanDemoApp::sqsBlend->descSet[0]));
...
float* TreeControl::calculateLastPosition(float bend_R, float direction_degree, float pointX, float pointY,
                                          float pointZ)
{
...
void TreeControl::calculateCenterPosition(float bend_RIn, float direction_degreeIn)
{
    ...
    for (int i = 0; i < leafVector.size(); i++)
    {
        ...
        leafVector[i]->centerx = (float)(leafVector[i]->centerx + pianDistance * sin(toRadians(direction_degreeIn)));
        //计算当前中心点X 坐标
        leafVector[i]->centerz = (float)(leafVector[i]->centerz + pianDistance * cos(toRadians(direction_degreeIn)));
        //计算当前中心点Z 坐标
    }
}
