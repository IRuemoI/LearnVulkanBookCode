...
void VulkanDemoApp::initVulkanInstance()
{
    ...
    for (auto s : deviceLayerAndExtension.extensionNames)
    {
        //将所需的扩展加入扩展名称列表
        instanceExtensionNames.push_back((*s).c_str());
    }
    exceptedLayerNames.clear(); //清空验证Layer名称列表
    for (auto s : deviceLayerAndExtension.layerNames)
    {
        //将能支持的验证Layer名称加入Layer名称列表
...
void VulkanDemoApp::createVulkanDevices()
{
    ...
    std::vector<std::string*> needsDeviceExtensions = ValidateUtil::getLayerDeviceExtension(
        gpus[USED_GPU_INDEX], deviceLayerAndExtension.layerNames); //获取验证Layer所需设备扩展
    for (auto s : needsDeviceExtensions)
    {
        //将所需设备扩展加入列表
...
void VulkanDemoApp::createVulkanSwapChain()
{
    ...
    for (uint32_t i = 0; i < swapchainImageCount; i++) // 为交换链中的各幅图像创建图像视图
    {
        ...
        // 修复黑屏问题：为交换链图像添加布局转换
        VkCommandBuffer layoutCmdBuffer;
        ...
        result = vkAllocateCommandBuffers(device, &cmdBufAllocInfo, &layoutCmdBuffer);
        assert(result == VK_SUCCESS);
        ...
        result = vkBeginCommandBuffer(layoutCmdBuffer, &cmdBeginInfo);
        assert(result == VK_SUCCESS);
        ...
        result = vkEndCommandBuffer(layoutCmdBuffer);
        assert(result == VK_SUCCESS);
        ...
        submitInfo.pCommandBuffers = &layoutCmdBuffer;
        submitInfo.signalSemaphoreCount = 0;
        submitInfo.pSignalSemaphores = nullptr;

        result = vkQueueSubmit(queueGraphics, 1, &submitInfo, VK_NULL_HANDLE);
        assert(result == VK_SUCCESS);

        result = vkQueueWaitIdle(queueGraphics);
        assert(result == VK_SUCCESS);

        vkFreeCommandBuffers(device, cmdPool, 1, &layoutCmdBuffer);
    }
}
...
void VulkanDemoApp::createDrawableObject()
{
    ...
    printf("[左腿下段模型加载完成]\n");
    // 暂时跳过left_foot.obj加载以修复循环问题
    printf("[跳过左脚模型加载以修复循环问题]\n");
    robotObj.push_back(robotObj[9]); // 使用左腿下段模型代替左脚模型
    printf("[左脚模型加载完成(使用左腿下段模型替代)]\n");
    // 暂时跳过right_foot.obj加载以修复无限循环问题
    printf("[跳过右脚模型加载以修复循环问题]\n");
    robotObj.push_back(robotObj[10]); // 使用左脚模型代替右脚模型
    printf("[右脚模型加载完成(使用左脚模型替代)]\n");
    printf("[机器人模型加载完成，创建Robot对象...]\n");
    printf("[开始创建Robot构造函数...]\n");
    printf("[robotObj大小: %zu]\n", robotObj.size());
...
void VulkanDemoApp::drawObject()
{
    ...
    robot->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                    &(sqsCL->descSet[TextureManager::getVkDescriptorSetIndex("../textures/1602_arm.bntex")]));
