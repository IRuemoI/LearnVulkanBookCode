...
void VulkanDemoApp::createDrawableObject()
{
    ...
    objectForDraw = new DrawableObjectCommon(ObjectData::vdata, ObjectData::dataByteCount, ObjectData::vCount, device,
                                             memoryProperties); // 创建待绘制对象
}
...
void VulkanDemoApp::drawObject()
{
    ...
    objectForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipelines[topologyWay], &(sqsCL->descSet[0]));
    // 绘制目标对象
