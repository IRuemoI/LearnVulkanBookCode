...
void BnggdhDraw::initVertexBuffer()
{
    ...
    bool flag = memoryTypeFromProperties(*memoryPropertiesPointer, vmem_reqs.memoryTypeBits, requirements_mask,
...
void BnggdhDraw::initIndexBuffer()
{
    ...
    VkFlags requirements_mask = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
    bool flag = memoryTypeFromProperties(*memoryPropertiesPointer, imem_reqs.memoryTypeBits, requirements_mask,
                                         &alloc_info.memoryTypeIndex);
    if (flag)
    {
        printf("确定内存类型成功 类型索引为%d\n", alloc_info.memoryTypeIndex);
