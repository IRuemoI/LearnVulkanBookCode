...
void setImageLayoutCube(VkCommandBuffer cmd, VkImage image, VkImageAspectFlags aspectMask,
...
void TextureManager::initCubemapTextures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu,
...
{
    //立方图纹理不能使用VK_IMAGE_TILING_LINEAR
    ...
    setImageLayoutCube(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    //拷贝纹理
    ...
    setImageLayoutCube(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    //结束一级命令缓冲
    result = vkEndCommandBuffer(cmdBuffer);
    //提交给队列执行
                                  ...
                                  VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i < texNames.size(); i++)
    {
        ...
        else if (sa[1].compare("bntexcube") == 0)
        {
            ...
            initCubemapTextures(texName, device, gpu, memoryProperties, cmdBuffer, queueGraphics,
                                VK_FORMAT_R8G8B8A8_UNORM, ctdo);
        }
    }
}
