...
void Flare::update(float lx, float ly)
{
    //更新光晕元素位置和尺寸的方法
    float currDis = (float)sqrt(lx * lx + ly * ly); //计算光源在屏幕上的投影位置距屏幕中心点的距离
    float currScale = Constant::SCALE_MIN + (Constant::SCALE_MAX - Constant::SCALE_MIN) * (1 - currDis /
        Constant::DIS_MAX); //计算尺寸缩放系数
