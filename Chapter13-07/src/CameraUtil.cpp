...
vector<float> CameraUtil::calLightScreen(float ratio)
{
    //计算屏幕光源位置
...
{
    float xStep = float(
        -goBack * sin(degree * 3.1415926535898 / 180) - leftRight * sin((degree + 90) * 3.1415926535898 / 180));
    float zStep = float(
        -goBack * cos(degree * 3.1415926535898 / 180) - leftRight * cos((degree + 90) * 3.1415926535898 / 180));
    tx = tx + xStep;
    tz = tz + zStep;
    calCamera();
}
