...
void VulkanDemoApp::createDrawableObject()
{
    ...
    guangForDraw = new TexDrawableObject(FlatData::vdata, FlatData::dataByteCount, FlatData::vCount, device,
                                         memoryProperties);
    flare = new Flare();
}
...
void VulkanDemoApp::drawSky()
{
    ...
    texForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSkyBox(
...
void VulkanDemoApp::drawObject()
{
    ...
    if (lpx < Constant::RATIO && lpy < 1)
    {
        ...
        for (auto ss : flare->sFl)
        {
            ...
            guangForDraw->drawSelf(cmdBuffer, sqsCube->pipelineLayout, sqsCube->pipeline,
                                   &(sqsCube->descSet[TextureManager::getVkDescriptorSetIndexForGuang(
                                       guangStr[ss->idIn])]), ss->color); //绘制光晕元素
