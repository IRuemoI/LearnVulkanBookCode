...
void VulkanDemoApp::createDrawableObject()
{
    ...
    cirForDraw = new DrawableObjectCommonLight(CircleData::vdata, CircleData::dataByteCount, CircleData::vCount,
                                               CircleData::idata, CircleData::indexByteCount, CircleData::iCount,
                                               device, memoryProperties); // 创建绘制用待绘制对象
}
...
void VulkanDemoApp::drawObject()
{
    ...
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0,
                         CircleData::iCount); //绘制正十边形
    MatrixState3D::popMatrix(); //恢复现场

    MatrixState3D::pushMatrix(); //保护现场
    MatrixState3D::translate(0, -50, 0); //沿Y 轴负方向平移50
    cirForDraw->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline, &(sqsCL->descSet[0]), 0,
                         CircleData::iCount / 2 + 1); //绘制正十边形的下半部分
    MatrixState3D::popMatrix(); //恢复现场
