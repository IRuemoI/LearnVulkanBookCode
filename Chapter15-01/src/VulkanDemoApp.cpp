...
void VulkanDemoApp::drawSceneToTex() //将场景绘制到5 幅纹理的方法
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        MatrixState3D::setCamera(0, 0, czCurrTemp - TIMESPAN * i, 0, 0, targetzCurrTemp - TIMESPAN * i, 0, 1, 0);
        //设置摄像机(摄像机Z 坐标每幅纹理不同)
        ...
        landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                              &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndexForSky(
                                  tp + (string)("grass.bntex"))])); //绘制云天空
        ...
        sky->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                      &(sqsCT->descSet[
                          TextureManager::getVkDescriptorSetIndexForSky(tp + (string)("sky_cloud.bntex"))]));
