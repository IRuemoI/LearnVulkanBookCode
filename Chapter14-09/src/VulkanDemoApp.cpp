...
{
    VkFormat colorFormat[OUT_TEX_COUNT] = {
        VK_FORMAT_R8G8B8A8_UNORM, VK_FORMAT_R32G32B32A32_SFLOAT, VK_FORMAT_R32G32B32A32_SFLOAT,
...
void VulkanDemoApp::drawSceneToTex()
{
    ...
    carBObjForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                             &(sqsCTL->descSet[
                                 TextureManager::getVkDescriptorSetIndex(tp + (string)("carBody.bntex"))]));
    carWObjForDraw->drawSelf(cmdBuffer, sqsCTL->pipelineLayout, sqsCTL->pipeline,
                             &(sqsCTL->descSet[
                                 TextureManager::getVkDescriptorSetIndex(tp + (string)("carWheel.bntex"))]));
    MatrixState3D::popMatrix();
    MatrixState3D::pushMatrix();
