...
void MS3DModel::initMs3dInfo(unsigned char* binaryData)
{
    //加载ms3d 文件信息的方法
...
void MS3DModel::initVertexBuffer(VkPhysicalDeviceMemoryProperties& memoryProperties)
{
    ...
    for (int i = 0; i < groups.size(); i++)
    {
        ...
        bool flag = memoryTypeFromProperties(memoryProperties, mem_reqs[i].memoryTypeBits, requirements_mask,
...
void MS3DModel::updateAllVectexs()
{
    //更新所有顶点位置数据的方法
    for (MS3DVertex* vtHelper : vertexs)
    {
        //对模型中的所有顶点进行遍历
        if (vtHelper->getBone() == -1)
        {
            //若无关节控制
