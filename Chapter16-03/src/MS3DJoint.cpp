...
{
    //根据当前播放时间和关键帧数据进行平移数据插值计算的方法
    int index = 0; //初始化索引为0
    int size = count_pos; //得到平移关键帧的数量
    while (index < size && positions[index]->getTime() < time)
    {
        //根据当前时间计算用于插值的结束关键帧索引
        index++; //关键帧索引加1
    }
    if (index == 0)
    {
...
{
    //根据当前播放时间和关键帧数据进行旋转数据插值计算的方法
    int index = 0; //初始化索引为0
    int size = count_rot; //获取旋转关键帧的数量
    while (index < size && rotates[index]->getTime() < time)
    {
        //根据当前时间计算用于插值的结束关键帧索引
        index++; //关键帧索引加1
    }
