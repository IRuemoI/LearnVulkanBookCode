...
void VulkanDemoApp::flushTexToDesSet() const
{
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        sqsDT->writes[3].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNames[i * 3 + 2]]);
        //写入属性对应的纹理信息(灰度图纹理)
...
void VulkanDemoApp::drawObject()
{
    ...
    landForDraw->drawSelf(cmdBuffer, sqsDT->pipelineLayout, sqsDT->pipeline,
                          &(sqsDT->descSet[
                              TextureManager::getVkDescriptorSetIndex("../textures/0902_grassMipMap.bntex")]));
