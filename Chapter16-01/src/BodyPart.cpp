...
void BodyPart::updateBone()
{
    ...
    if (father != nullptr)
    {
        //若父骨骼不为空，则此骨骼在世界坐标系中的变换矩阵为自身矩阵乘以父骨骼矩阵
        Matrix::multiplyMM(mWorld, 0, father->mWorld, 0, mFather, 0);
    }
    else
    {
        //若父骨骼为空，则此骨骼在世界坐标系中的变换矩阵为自己的变换矩阵
                        ...
                        VkDescriptorSet* desSetPointer)
{
    if (index == 2)
    {
        desSetPointer = &(VulkanDemoApp::sqsCL->descSet[TextureManager::getVkDescriptorSetIndex(
