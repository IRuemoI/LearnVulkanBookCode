...
Dashboard2DObject::Dashboard2DObject(float* vdataIn, int dataByteCount, int vCountIn, VkDevice& device,
...
void Dashboard2DObject::drawSelf(VkCommandBuffer& cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
                                 VkDescriptorSet* desSetPointer)
{
    ...
    vkCmdPushConstants(cmd, pipelineLayout, VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(float) * 16, sizeof(float) * 8,
                       pushConstantData + 16);
