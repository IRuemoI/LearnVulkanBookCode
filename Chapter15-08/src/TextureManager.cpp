...
void convertImageLayout(VkCommandBuffer cmd, VkImage image, VkImageAspectFlags aspectMask,
...
std::vector<std::string> TextureManager::texNames = {
    tp + (string)("Head_256x256x256.bn3dtex"), tp + (string)("Head_256x256x256.bntex")
};
...
void TextureManager::init_SPEC_2D_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu,
                                           ...
                                           TexDataObject* ctdo)
{
    ...
    if (needStaging)
    {
        ...
        flag = memoryTypeFromProperties(memoryProperties, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
        ...
        convertImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
        vkCmdCopyBufferToImage(cmdBuffer, tempBuf, textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                               &bufferCopyRegion);
        convertImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void TextureManager::init_SPEC_3D_Textures(std::string texName, VkDevice& device, VkPhysicalDevice& gpu,
                                           VkPhysicalDeviceMemoryProperties& memoryProperties,
                                           VkCommandBuffer& cmdBuffer, VkQueue& queueGraphics, VkFormat format,
                                           ThreeDTexDataObject* ctdo)
{
    ...
    memoryTypeFromProperties(memoryProperties, memReqs.memoryTypeBits,
    ...
    convertImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_UNDEFINED,
                       VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
    vkCmdCopyBufferToImage(cmdBuffer, stagingBuffer, textureImage, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                           &bufferCopyRegion);
    convertImageLayout(cmdBuffer, textureImage, VK_IMAGE_ASPECT_COLOR_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                       VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
    result = vkEndCommandBuffer(cmdBuffer);
...
void TextureManager::initTextures(VkDevice& device, VkPhysicalDevice& gpu,
                                  VkPhysicalDeviceMemoryProperties& memoryProperties, VkCommandBuffer& cmdBuffer,
                                  VkQueue& queueGraphics)
{
    ...
    for (int i = 0; i < texNames.size(); i++)
    {
        ...
        if (sa[1].compare("bntex") == 0)
        {
            ...
            init_SPEC_2D_Textures(texName, device, gpu, memoryProperties, cmdBuffer, queueGraphics,
                                  VK_FORMAT_R8G8B8A8_UNORM, ctdo);
        }
        else if (sa[1].compare("bn3dtex") == 0)
        {
            ThreeDTexDataObject* ctdo = FileUtil::load3DTexData(texNames[i]);
            init_SPEC_3D_Textures(texName, device, gpu, memoryProperties, cmdBuffer, queueGraphics, VK_FORMAT_R16_UNORM,
                                  ctdo);
        }
    }
}
