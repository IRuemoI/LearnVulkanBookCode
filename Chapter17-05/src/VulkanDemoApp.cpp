...
void VulkanDemoApp::creat_QueryPool()
{
    //创建查询池的方法
...
void VulkanDemoApp::destroy_QueryPool()
{
    //销毁查询池
...
void VulkanDemoApp::drawObject()
{
    ...
    //重置查询池  查询池中的每个查询都被标记为可用或不可用。 最初，池中的所有查询处于未定义状态，并且在可以使用任何查询之前，您需要重置池
...
void VulkanDemoApp::drawOcclusionObj()
{
    ...
    for (int i = 0; i < CR - 1; i++)
    {
        ...
        objObjectZDT->drawSelf(cmdBuffer, sqsCL->pipelineLayout, sqsCL->pipeline,
                               &(sqsCL->descSet[
                                   TextureManager::getVkDescriptorSetIndex("../textures/1705_muwen.bntex")]));
        MatrixState3D::popMatrix();
    }
