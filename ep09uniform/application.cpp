...
namespace FF
{
    ...
    {
        mWindow = Wrapper::Window::create(mWidth, mHeight); //从自定义窗口类中创建窗口
    }
    ...
    void Application::initVulkan()
    {
        ...
        mSwapChain = Wrapper::SwapChain::create(mDevice, mWindow, mSurface); //创建交换链
        ...
        mPipeline = Wrapper::Pipeline::create(mDevice, mRenderPass); //创建图形流水线
        createPipeline();
    ...
    void Application::createPipeline()
    {
        ...
        mPipeline->mAssemblyState.primitiveRestartEnable = VK_FALSE; //图元重启：在绘制完成一个图元之后，需要重新开始绘制另一个图元时使用
        ...
        mPipeline->mRasterState.depthBiasEnable = VK_FALSE; //深度偏移，可以解决因为两图元深度值相同导致的显示问题
    ...
    void Application::createCommandBuffers()
    {
        for (int i = 0; i < mSwapChain->getImageCount(); ++i)
        {
            ...
            mCommandBuffers[i]->bindDescriptorSet(mPipeline->getLayout(),
                                                  mUniformManager->getDescriptorSet(mCurrentFrame));
