...
std::vector<std::string*> ValidateUtil::getLayerDeviceExtension(VkPhysicalDevice& gpu,
                                                                std::vector<std::string*> supportedlayerNames)
{
    ...
    for (int i = 0; i < layerList.size(); i++)
    {
        ...
        propertiesList.resize(propertyCount); //调整列表长度
        vkEnumerateDeviceExtensionProperties(gpu, lp.layerName, &propertyCount, propertiesList.data());
        //填充当前验证Layer对应设备扩展属性列表
        for (auto ep : propertiesList)
        {
            //遍历设备扩展属性列表
            printf("  所需设备扩展:%s\n", ep.extensionName);
...
void ValidateUtil::createDebugReportCallbackSelf(VkInstance instance)
{
    //创建调试报告回调相关
    ...
    VkResult result = dbgCreateDebugReportCallback(instance, &dbgReportCreateInfo, nullptr, &debugReportCallback);
    //创建调试报告回调实例
