...
void DrawableObjectCommonLight::createIndexBuffer(int indexByteCount, VkDevice& device,
                                                  VkPhysicalDeviceMemoryProperties& memoryProperties)
{
    ...
    //获取所需内存类型索引
    bool index_flag = memoryTypeFromProperties(memoryProperties, index_mem_reqs.memoryTypeBits, index_requirements_mask,
...
void DrawableObjectCommonLight::drawSelf(VkCommandBuffer& cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
                                         VkDescriptorSet* desSetPointer, uint32_t sIndex, uint32_t eIndex)
{
