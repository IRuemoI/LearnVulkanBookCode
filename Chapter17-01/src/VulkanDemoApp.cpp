...
VulkanDemoApp::get_required_extensions()
{
    ...
    // 申请glfwExtensionCount个用于存储扩展名称的向量单位空间
    return extensions;
}
...
void VulkanDemoApp::createDrawableObject()
{
    ...
    open = new DrawableObjectCommonLight(vdataIn, 48 * 4, 8, dataIndex, 9 * sizeof(uint16_t), 9, device,
                                         memoryProperties); //创建有效使用图元重启时的绘制物体
    ...
    close = new DrawableObjectCommonLight(vdataIn, 48 * 4, 8, dataIndex, 8 * sizeof(uint16_t), 8, device,
                                          memoryProperties); //创建未有效使用图元重启时的绘制物体
}
