...
public:
    ...
    DrawableObjectCommonLight(float* vdataIn, int dataByteCount, int vCountIn, uint16_t* idataIn, int indexByteCount,
                              int iCountIn, VkDevice& device,
    ...
    void drawSelf(VkCommandBuffer& secondary_cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
                  VkDescriptorSet* desSetPointer, uint32_t sIndex, uint32_t eIndex); //绘制方法

private:
    void createVertexBuffer(int dataByteCount, VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryProperties);
    //创建顶点数据缓冲的方法
    void createIndexBuffer(int indexByteCount, VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryProperties);
    //创建索引数据缓冲的方法
};
