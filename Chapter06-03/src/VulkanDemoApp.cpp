...
void VulkanDemoApp::drawObject()
{
    ...
    if (texType == 0)
    {
        texTri->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                         &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)])); //绘制物体 0
    }
    //若 texType 为 1(采用 4×2 纹理坐标范围)
    else if (texType == 1)
    {
        texTri1->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)])); //绘制物体 1
    }
    //若 texType 为 2(采用 1×1 纹理坐标范围)
    else if (texType == 2)
    {
        texTri2->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex(textureName)])); //绘制物体 2
    }
