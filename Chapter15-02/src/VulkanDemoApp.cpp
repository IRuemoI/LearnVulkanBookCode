...
void VulkanDemoApp::destroyVulkanCommandBuffer()
{
    // 销毁命令缓冲的方法
    // 创建要释放的命令缓冲数组
...
void VulkanDemoApp::destroyVulkanSwapChain()
{
    // 销毁交换链相关的方法
...
void VulkanDemoApp::createDrawableObject()
{
    ...
    sky = new Dashboard2DObject(SkyCloudData::vdata, SkyCloudData::dataByteCount, SkyCloudData::vCount, device,
                                memoryProperties);
...
void VulkanDemoApp::flushUniformBufferForToScreen()
{
    float fragmentUniformData[33] = {
        mViewProjectionInverseMatrix[0], mViewProjectionInverseMatrix[1], mViewProjectionInverseMatrix[2],
        mViewProjectionInverseMatrix[3],
        mViewProjectionInverseMatrix[4], mViewProjectionInverseMatrix[5], mViewProjectionInverseMatrix[6],
        mViewProjectionInverseMatrix[7],
        mViewProjectionInverseMatrix[8], mViewProjectionInverseMatrix[9], mViewProjectionInverseMatrix[10],
        mViewProjectionInverseMatrix[11],
        mViewProjectionInverseMatrix[12], mViewProjectionInverseMatrix[13], mViewProjectionInverseMatrix[14],
        mViewProjectionInverseMatrix[15],
        mPreviousProjectionMatrix[0], mPreviousProjectionMatrix[1], mPreviousProjectionMatrix[2],
        mPreviousProjectionMatrix[3],
        mPreviousProjectionMatrix[4], mPreviousProjectionMatrix[5], mPreviousProjectionMatrix[6],
        mPreviousProjectionMatrix[7],
        mPreviousProjectionMatrix[8], mPreviousProjectionMatrix[9], mPreviousProjectionMatrix[10],
        mPreviousProjectionMatrix[11],
        mPreviousProjectionMatrix[12], mPreviousProjectionMatrix[13], mPreviousProjectionMatrix[14],
        mPreviousProjectionMatrix[15],
        2.0f
    };
    uint8_t* pData;
...
void VulkanDemoApp::drawSceneToTex() //将场景绘制到2 幅纹理的方法
{
    ...
    landForDraw->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                          &(sqsCT->descSet[
                              TextureManager::getVkDescriptorSetIndexForSky(tp + (string)("grass.bntex"))]));
