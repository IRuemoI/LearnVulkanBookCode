...
void VulkanDemoApp::createDrawableObject()
{
    ...
    {
        //遍历植物组
        trees[i] = new TexDrawableObject(TreeData::vData, TreeData::dataByteCount, TreeData::vCount, device,
                                         memoryProperties, position[i][0], position[i][2]);
    }
...
void VulkanDemoApp::drawObject()
{
    ...
    texRect->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                      &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0901_desert.bntex")]));
    MatrixState3D::popMatrix();
    for (int i = 0; i < 9; i++)
    {
        MatrixState3D::pushMatrix();
        trees[i]->drawSelf(cmdBuffer, sqsCT->pipelineLayout, sqsCT->pipeline,
                           &(sqsCT->descSet[TextureManager::getVkDescriptorSetIndex("../textures/0901_tree.bntex")]));
        MatrixState3D::popMatrix();
    }
