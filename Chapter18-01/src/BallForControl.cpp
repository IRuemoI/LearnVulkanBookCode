...
void BallForControl::go()
{
    ...
    for (int i = 0; i < VulkanDemoApp::allBall.size(); i++)
    {
        //遍历所有桌球判断是否相互发生碰撞
    ...
    if (xOffset < -TABLE_SIZE + BALL_R || xOffset > TABLE_SIZE - BALL_R)
    {
        //碰左挡板或右挡板
        vx = -vx; //速度的X 分量置反
        flag = true; //碰撞标志置true
    }
    else if (zOffset < -TABLE_SIZE + BALL_R || zOffset > TABLE_SIZE - BALL_R)
    {
        //碰前挡板或后挡板
        vz = -vz; //速度的Z 分量置反
        flag = true; //碰撞标志置true
