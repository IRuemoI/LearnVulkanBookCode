...
{
    //将场景绘制到纹理的方法
    rp_begin.framebuffer = selfTexFramebuffer; //将自定义帧缓冲设置为渲染过程的当前帧缓冲
    ...
    result = vkEndCommandBuffer(cmdBuffer); //结束渲染过程
...
void VulkanDemoApp::flushTexToDesSetForToScreen()
{
    for (int i = 0; i < TextureManager::texNamesForCommonTexLight.size(); i++)
    {
        ...
        sqsCTL->writes[3].pImageInfo = &(TextureManager::texImageInfoList[TextureManager::texNamesForCommonTexLight[
            i]]);
