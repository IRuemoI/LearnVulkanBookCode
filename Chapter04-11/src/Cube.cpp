...
Cube::Cube(VkDevice& device, VkPhysicalDeviceMemoryProperties& memoryProperties)
{
    //构造函数
    ColorRect::genVertexData(); //生成正方形顶点数据
    colorRect = new DrawableObjectCommon(ColorRect::vdata, ColorRect::dataByteCount, ColorRect::vCount, device,
                                         memoryProperties); //创建绘制对象(正方形
}

void Cube::drawSelf(VkCommandBuffer cmd, VkPipelineLayout& pipelineLayout, VkPipeline& pipeline,
